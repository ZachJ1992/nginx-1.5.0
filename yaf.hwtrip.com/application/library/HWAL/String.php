...
class HWAL_String {
    ...
    public static function isUTF8($str)
    {
       ...
       )*$%xs', $string);  // x 从模式中删除空白符和注释 s 使句点(.)匹配任何字符，包括换行符(\n)
    }
    /* }}} */
     ...
     * @return string
     */
    public static function mbSubstr($str, $start, $length, $suffix='...', $charset="utf-8")
    {
        //< Modified by wubingjie
        $_ret = mb_substr($str, $start, $length, $charset);
        $_len = mb_strlen($str, $charset);
        if($_len>$length && !empty($suffix))
            $_ret .= $suffix;
        return $_ret;
    }
    /* }}} */
    ...
    public static function randString($len=6, $type='',$addChars='')
    {
        ...
        switch($type) {
            ...
            case 4:
                ...
                break;
            default :
                // 默认去掉了容易混淆的字符oOLl,9gq,Vv,Uu,I1,要添加请使用addChars参数
                $chars='ABCDEFGHJKMNPQRSTWXYZabcdefhijkmnprstwxyz2345678'.$addChars;
                break;
        }
        ...
        if($type != 4) {
            ...
            //< 中文随机字
            for($i=0;$i<$len;$i++){
              $str.= self::mbSubstr($chars, floor(mt_rand(0, mb_strlen($chars, 'utf-8')-1)), 1);
            }
        }
        return $str;
    }
